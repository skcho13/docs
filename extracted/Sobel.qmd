...
theidentificationandestimationoftheeffect(s)ofparticularcauses—
...
comparison h(Y (!), Y (!)). Because each study unit can actually be
s s0
...
difficult to substantively justify. For example, consider the case of an
...
Fromthispointofview,allelsebeingequal,giventhechoicebetweena
...
justify these approachesdiffer, the propensity score(discussed below)
...
methods.Atthispoint,thereisalargestatisticalliteratureonmatching
...
0 1
needstosatisfytheconditionE(U j X,Z) ¼ E(U j X) ¼ 0(equivalently,
...
Rubin,D.B.1974.‘‘EstimatingCausalEffectsofTreatmentsinRandomizedand
